...
export default function LocalPrintAgentPanel({ restaurantId, printers = [] }) {
    ...
    // Same fix as the Android agent tab: Clean up only removes finished jobs, so
    // jobs stuck after a printer outage could only be removed in the database.
                        ...
                        <div className="flex gap-2">
                            ...
                            {stuckCount > 0 && (
                                <Button variant="outline" size="sm" onClick={handleClearStuck} disabled={clearingStuck}
                                    ...
                                    <Trash2 className="h-3.5 w-3.5 mr-1" />Clear stuck ({stuckCount})
                                </Button>
                            )}
                        </div>
                    </div>
