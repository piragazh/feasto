...
export default function CuisineFilter({ selectedCuisine, setSelectedCuisine }) {
    ...
    return (
        <div className="py-4 md:py-8">
            ...
            <div role="group" aria-label="Filter by cuisine" className="flex gap-2 md:gap-3 overflow-x-auto pb-3 md:pb-4 scrollbar-hide -mx-3 md:mx-0 px-3 md:px-0">
                {cuisines.map((cuisine) => {
                    const isActive = (cuisine.name === 'All' && !selectedCuisine) || selectedCuisine === cuisine.name;
                    return (
                    <motion.button
                        key={cuisine.name}
